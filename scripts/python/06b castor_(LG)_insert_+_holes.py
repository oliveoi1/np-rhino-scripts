...
TARGET_BLOCK = "ACC-CST-LG"

HOLE_SPACING_X_IN = 2.950
HOLE_SPACING_Y_IN = 1.770
...
def local_offset_point(insert_pt, xform, off_x, off_y):
    _, raw_x = get_local_axis_world(xform, "X")
    _, raw_y = get_local_axis_world(xform, "Y")

    raw_x.Unitize()
    raw_y.Unitize()

    x_axis = Rhino.Geometry.Vector3d(raw_y)
    y_axis = Rhino.Geometry.Vector3d(raw_x)
    y_axis *= -1.0
...
def main():
    ...
    hole_spacing_x = inches_to_doc_units(HOLE_SPACING_X_IN)
    hole_spacing_y = inches_to_doc_units(HOLE_SPACING_Y_IN)
    ...
    for src_id in source_ids:
        try:
            ...
            ccw_rot = Rhino.Geometry.Transform.Rotation(
                math.radians(0.0),
                axis_z,
                insert_pt_z
            )
            rs.TransformObject(new_id, ccw_rot, copy=False)

            rs.ObjectLayer(new_id, rs.ObjectLayer(src_id))
            placed += 1

            half_x = hole_spacing_x / 2.0
            half_y = hole_spacing_y / 2.0

            hole_points = [
                local_offset_point(insert_pt, xform,  half_y,  half_x),
                local_offset_point(insert_pt, xform, -half_y,  half_x),
                local_offset_point(insert_pt, xform, -half_y, -half_x),
                local_offset_point(insert_pt, xform,  half_y, -half_x),
            ]
